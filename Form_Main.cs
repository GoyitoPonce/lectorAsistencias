...
namespace UareUSampleCSharp
{
    public partial class Form_Main : Form
    {
        ...
        {
            InitializeComponent();
            LoadFmdsFromServerAsync().ConfigureAwait(false);
        }
        ...
        private async Task LoadFmdsFromServerAsync(){
            try
            {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        foreach (var employee in employees)
                        {
                            try
                            {
                                ...
                                string fingerprintXml = Encoding.UTF8.GetString(storedFingerprintBytes);
                                Fmd storedFmd = Fmd.DeserializeXml(fingerprintXml);

                                Fmds.Add(employee.pk_finger, storedFmd);


                            }
                            catch (FormatException formatEx)
                            {
                                MessageBox.Show($"Error al procesar la huella Base64 de Empleado ID {employee.pk_finger}: {formatEx.Message}");
                            ...
                            catch (Exception innerEx)
                            {
                                ...
                                MessageBox.Show($"Error al procesar huella para Empleado ID {employee.pk_finger}: {innerEx.Message}");
                            }
                        }
